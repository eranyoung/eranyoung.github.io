...
function draw() { 
    ...
    for(let i = 0; i < numberOfRows; i++) { 
        ...
        textAlign(CENTER);
        textSize(12);
        text(table.getString(i, 0).substring(0,2).toUpperCase(), c, pageHeight - 30);
        let rate = table.getNum(i, 1);
        fill(168, 50, 50);
        let circleHeight = pageHeight-50-rate*(rate/ppt);
        circle(c, circleHeight, 15, 15); 
